...
class ReliefFSelector:
    ...
    def fit(self,
        ...
        """
        Fit the ReliefF feature selector to the training data
        """
        logger.info("Starting ReliefF feature selection...")
        ...
        # Fit ReliefF
        self.feature_selector.fit(x_train.values if isinstance(x_train, pd.DataFrame) else x_train,
                                y_labels)
        ...
        self.features_df = pd.DataFrame({
            'feature': feature_names,
            'score': np.abs(self.feature_selector.feature_importances_)  # Use absolute values
        })
        self.features_df = self.features_df.sort_values('score', ascending=False)
